...
LRESULT CALLBACK WndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_CREATE:
	{
		...
		CreateWindow(L"button", L"Ok", WS_CHILD | WS_VISIBLE | BS_PUSHBUTTON, 120, 0, 100, 25, hWnd, (HMENU)4001, hInst, NULL);
	}
	break;
    case WM_COMMAND:
        {
			...
			{
				TCHAR nNum[256];
				GetWindowText(GetDlgItem(hWnd, 3001), nNum, 256);
				int num, a = 20;
				num = _wtoi(nNum);
				...
				for (int i = 0; i < 9; i++) {
					swprintf_s(szBuf, 256, L"%d * %d = %d ", num, i + 1,num * (i + 1));
					TextOut(hdc, 0, 50 + a, szBuf, wcslen(szBuf));
					a += 20;
				}
				ReleaseDC(hWnd, hdc);
			}
