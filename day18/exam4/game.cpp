...
S_ObjectBullet *g_pBullet_List2[MAX_BULLET_LIST];
...
{
	int j;
	for (j = 0; j < MAX_BULLET_LIST; j++) {
		if (g_pBullet_List2[j] == NULL) {
			S_ObjectBullet *ptr2 = (S_ObjectBullet *)malloc(sizeof(S_ObjectBullet));
			ObjectBullet_Setup(ptr2, irr::core::vector2df(-120, 0), g_objPlayer.m_vPosition, 8, rand() % 50 + 20);
			g_pBullet_List2[j] = ptr2;
			return j;
		}
	}
...
void ClearDeadBulletObj()
{
	...
	for (i = 0; i < MAX_BULLET_LIST; i++) {
		...
		if (g_pBullet_List2[i] != NULL && g_pBullet_List2[i]->m_nFSM == 999) {
			free(g_pBullet_List2[i]);
			g_pBullet_List2[i] = NULL;
		}
	}
}
...
void OnLoop(double fDelta)
{
	...
	for (int i = 0; i < MAX_BULLET_LIST; i++) {
		...
		S_ObjectBullet *ptr = g_pBullet_List2[i];
		if (ptr != NULL) {
			ObjectBullet_OnApply(ptr, fDelta);
		}
	}
